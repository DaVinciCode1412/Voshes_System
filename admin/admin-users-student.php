...
<?php
    ...
    if (isset($_POST['search'])) {
        ...
        $sql = "select * from user_info where first_name = '$fname' and last_name = '$lname' and position = 'student' and status = 'normal' ";
        $result = $search_con->query($sql);
        $pending = False;
    }
    elseif(isset($_POST['pending_students'])){
        $sql = "select * from user_info where type = 'pending' ";
        $result = $search_con->query($sql);
        $pending = True;
    }
    else{
        $sql = "select * from user_info where position = 'student' and status = 'normal' order by user_id";
        $result = $search_con -> query($sql);
        $pending = False;
    }
?>
                ...
                <div class="container-fluid">
                    <div class="d-sm-flex justify-content-between align-items-center mb-4">
                        <h3 class="text-dark" style="font-family: ABeeZee, sans-serif;color: rgb(29,29,31);margin-bottom: -5px;"><strong>Student Section</strong></h3>
                    </div>
                </div>
                <div class="row" style="margin-left: 0px;margin-right: 0px;padding-top: 7px;">
                    <div class="input-group container" >
                        <button class="btn btn-success" data-toggle="modal" data-target="#modal1" type="button" >Add Student</button>
                        <div class="col">
                            <div class="row">
                                <form id="form1" action="admin-users-student.php" method="post" class="col-lg-9 ">
                                ...
                                <form class="col-lg-3 text-right" action="admin-users-student.php" method="post">
                                    <input type="submit" name="pending_students" class="btn btn-success" value="Pending Students">
                                </form>
                            </div>
                        </div>
                    </div>
                            ...
                            if ($result -> num_rows > 0 ) {
                                while ($row = $result -> fetch_assoc()) {
                                    echo "<form action='save.php' method='post' ><tr><td><input type='text' style='background-color:#f2faff;border:none;' name='id' value='" . $row['user_id'] . "' class='form-control'></td><td><input type='text' style='background-color:#f2faff;border:none;' name='fname' value='" . $row['first_name'] . "' class='form-control'></td><td><input type='text' style='background-color:#f2faff;border:none;' name='lname' value='" . $row['last_name'] . "' class='form-control'></td><td><input type='text' style='background-color:#f2faff;border:none;' name='position' value='" . $row['position'] . "' class='form-control'></td><td><input type='text' style='background-color:#f2faff;border:none;' name='email' value='" . $row['email'] . "' class='form-control'></td>";
                                        if (!$pending) {
                                            echo "<td><input type='submit' name='edit_student' class='btn btn-primary' value='SAVE'><input type='submit' name='delete_student' class='btn btn-danger' value='ARCHIVE'></td>";
                                        }
                                        else{
                                            echo "<td><input type='submit' name='approve' class='btn btn-primary' value='APPROVE'><input type='submit' name='decline' class='btn btn-danger' value='DECLINE'></td>";
                                        }
                                    echo "</tr></form>";
                                }
                            }
                ...
                <div class="modal-dialog modal-dialog-centered modal-dialog-scrollable" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-footer" style="width: 100%;background: rgb(220,228,246);">
                            <div class="form-group"><button class="btn btn-danger" type="button" data-dismiss="modal" style="margin-right: 11px;">Close</button><button class="btn btn-success" type="submit" name="student" style="background: rgb(46,169,80);">Save</button></div>
                        </div>
                    </div>
                </div>
            </div>
